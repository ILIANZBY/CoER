...
def initialize_global_process_group(timeout_second=36000):
    ...
    if init_method is None:
        ...
        init_method = f"tcp://{_bracket_ipv6(addr)}:{port}"
...
def _bracket_ipv6(addr: str) -> str:
    """Wrap an IPv6 address in brackets for use in TCP URLs. IPv4 returned as-is."""
...
def initialize_global_process_group_ray(timeout_second=None, backend=None):
    ...
    if not torch.distributed.is_initialized():
        ...
        # A Ray worker group wholly contained on one node does not need a
        # network rendezvous.  Using the probe-and-release MASTER_PORT assigned
        # by Ray is racy when replicas are restarted: another TCPStore may
        # claim the port before init_process_group, leaving every actor in an
        # unbounded TCP retry loop.  All ranks in a RayWorkerGroup share
        # WG_PREFIX, so a group-scoped local FileStore removes that contention
        # without changing the later NCCL data-plane groups used for weight
        # transfer.  RAY_LOCAL_WORLD_SIZE is the variable RayWorkerGroup sets;
        # LOCAL_WORLD_SIZE is retained as a compatibility fallback.
        if init_method is None:
            local_world_size = int(
                os.environ.get("RAY_LOCAL_WORLD_SIZE", os.environ.get("LOCAL_WORLD_SIZE", "1"))
            )
            if local_world_size == world_size and world_size > 1:
                group_identity = "|".join(
                    (
                        os.environ.get("WG_PREFIX", ""),
                        os.environ.get("MASTER_ADDR", ""),
                        os.environ.get("MASTER_PORT", ""),
                        str(world_size),
                    )
                )
                group_digest = hashlib.sha256(group_identity.encode("utf-8")).hexdigest()[:24]
                store_path = os.path.join(tempfile.gettempdir(), f"verl_ray_pg_{group_digest}")
                init_method = f"file://{store_path}"
                os.environ["DIST_INIT_METHOD"] = init_method
            elif rank == 0 and world_size == 1:
                fd, store_path = tempfile.mkstemp(prefix=f"verl_ray_pg_{os.getpid()}_")
                os.close(fd)
        ...
        max_retries = 5 if (rank == 0 and world_size == 1) else 1
        for attempt in range(max_retries):
            try:
                ...
                if "EADDRINUSE" in str(e) and attempt < max_retries - 1:
                    ...
                    addr = os.environ.get("MASTER_ADDR", "127.0.0.1")
                    init_method = f"tcp://{_bracket_ipv6(addr)}:{new_port}"
                    os.environ["DIST_INIT_METHOD"] = init_method
