        ...
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            if self.should_load_model:
                ...
                if fsdp_version(self.model) == 2:
                    # FSDP2: checkpoint contains local shards (saved via DTensor.to_local()).
                    # model.load_state_dict() expects full logical shapes and will fail.
                    # Instead, copy local shard data directly into each DTensor's local storage.
                    # Use torch.no_grad() because to_local() returns a view with autograd history
                    # (_ToTorchTensorBackward) that forbids inplace modification.
                    from torch.distributed._tensor import DTensor

                    raw_sd = self.model.state_dict(keep_vars=True)
                    with torch.no_grad():
                        for k, loaded_v in model_state_dict.items():
                            if k not in raw_sd:
                                continue
                            current_v = raw_sd[k]
                            if isinstance(current_v, DTensor):
                                local_t = current_v.to_local()
                                local_t.copy_(loaded_v.to(local_t.device))
                            else:
                                current_v.copy_(loaded_v.to(current_v.device))
                else:
            ...
            if self.should_load_optimizer:
                ...
                if fsdp_version(self.model) == 2:
                    # FSDP2 + CPUOffloadPolicy: manually reconstructing DTensor optimizer
                    # state is incompatible with FSDP2's internal offload hooks (device
                    # mismatch between exp_avg on CPU and grad on CUDA during step).
                    # Skip optimizer state restore — FSDP2 will naturally initialize fresh
                    # state on the first optimizer.step(). Model weights are correctly loaded
                    # which is what matters; Adam momentum rebuilds within a few steps.
                    log_with_rank(
                        f"FSDP2: skipping optimizer state restore (incompatible with CPUOffloadPolicy). "
                        f"Optimizer state will be re-initialized on first step.",
                        rank=self.rank,
                        logger=logger,
                    )
                else:
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_model:
                    # FSDP2 + CPUOffloadPolicy: both .detach() and .data trigger
                    # "storage device != data device" errors on DTensors.
                    # Fix: use keep_vars=True then extract local shard via to_local().
                    if fsdp_version(self.model) == 2:
                        from torch.distributed._tensor import DTensor
                        ...
                        for k, v in raw_sd.items():
                            if isinstance(v, DTensor):
                                model_state_dict[k] = v.to_local().detach().clone()
                            else:
                ...
                if self.should_save_optimizer:
                    if fsdp_version(self.model) == 2:
                        ...
                        for k, v in raw_opt_sd.items():
                            if k == "state":
                                optimizer_state_dict[k] = {}
                                for param_id, param_state in v.items():
                                    new_state = {}
                                    for sk, sv in param_state.items():
                                        if isinstance(sv, DTensor):
                                            new_state[sk] = sv.to_local().detach().clone()
                                        elif isinstance(sv, torch.Tensor):
                                            new_state[sk] = sv.clone()
                                        else:
                                            new_state[sk] = sv
                                    optimizer_state_dict[k][param_id] = new_state
                            else:
