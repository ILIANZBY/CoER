...
def apply_chat_template(
    ...
    **kwargs,
    ...
    try:
        ...
        # If messages start with system, insert dummy_user after system to avoid
        # "System message must be at the beginning" error
        if messages and messages[0].get("role") == "system":
            patched_messages = messages[:1] + dummy_user_message + messages[1:]
        else:
            patched_messages = dummy_user_message + messages

        # Build prefix: system (if any) + dummy_user
        prefix_messages = patched_messages[: len(patched_messages) - len(messages) + (1 if messages and messages[0].get("role") == "system" else 0)]
        if not prefix_messages:
            prefix_messages = dummy_user_message

        dummy_user_prefix = processor.apply_chat_template(
            messages[:1] + dummy_user_message if messages and messages[0].get("role") == "system" else dummy_user_message,
            tokenize=tokenize,
