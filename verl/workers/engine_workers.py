...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: TrainingWorkerConfig):
        ...
        # Load custom modules (e.g. custom policy loss) in worker processes
        import importlib
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        # Offload actor/ref models to CPU before vLLM initialization so that
        # gpu_memory_utilization can be set higher (e.g. 0.7) without OOM.
        if "actor" in self.role and self.actor.engine.is_param_offload_enabled:
            self.actor.engine.to("cpu", model=True, optimizer=False, grad=False)
        if "ref" in self.role and hasattr(self, "ref") and self.ref.engine.is_param_offload_enabled:
