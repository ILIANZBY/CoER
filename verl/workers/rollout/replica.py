...
class RolloutReplica(ABC):
    ...
    async def init_standalone(self):
        ...
        # Both placement groups and named Ray actors are cluster-global.  A
        # failed job can leave either one registered while a retry starts, so
        # use one allocation ID consistently for this standalone replica.
        standalone_allocation_id = uuid.uuid4().hex[:8]
        if self.is_reward_model:
        ...
        # Placement-group names are cluster-global.  A failed Ray job can leave
        # its non-detached groups behind long enough to collide with a retry
        # that uses the same rollout role/replica name.  Keep the readable
        # prefix, but make every standalone allocation distinct.  The manager
        # explicitly removes the old groups before an in-job replica restart.
        resource_pool_name = f"{resource_pool_name}_{standalone_allocation_id}"
        resource_pool_spec = {
        ...
        name_prefix = f"{name_prefix}_{standalone_allocation_id}"
        worker_group = RayWorkerGroup(
            resource_pool=self.resource_pool,
            ray_cls_with_init=self.get_ray_class_with_init_args(),
            bin_pack=False,
