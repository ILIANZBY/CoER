...
# vLLM 0.17 ships a Qwen3.5 text CausalLM implementation but does not expose
# it in the model registry.  Without this alias, a pure-text checkpoint falls
# back to Transformers' Qwen3.5 VLM class solely because its normalized outer
# config is ``qwen3_5``.  Register the built-in CausalLM under a distinct
# architecture name before any AsyncLLM config is built.
...
_VERL_VLLM_MP_PORT_ENV = "VERL_VLLM_MP_PORT"
_VERL_VLLM_MP_PORT_START = 20000
_VERL_VLLM_MP_PORT_END = 30000


def _reserve_vllm_mp_port(seed: int) -> tuple[int, socket.socket]:
    """Reserve a node-local port for one vLLM multiprocess executor.

    vLLM's multiprocess executor normally probes an ephemeral port, closes
    the probe socket, and only binds the TCPStore after spawning its workers.
    Concurrent standalone replicas can therefore select the same port.  Keep
    a real socket reservation in the Ray server actor until immediately before
    EngineCore is spawned.  Use the IPv4 loopback interface because that is
    what vLLM's local multiprocess executor binds, even on an IPv6 Ray cluster.
    """

    port_count = _VERL_VLLM_MP_PORT_END - _VERL_VLLM_MP_PORT_START
    first_offset = (seed * 257) % port_count
    for attempt in range(port_count):
        port = _VERL_VLLM_MP_PORT_START + (first_offset + attempt) % port_count
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            # Do not enable SO_REUSEADDR: this socket is an exclusive
            # reservation, not a listener being restarted after TIME_WAIT.
    ...
    raise RuntimeError(
        f"Unable to reserve a vLLM multiprocess port in "
        f"[{_VERL_VLLM_MP_PORT_START}, {_VERL_VLLM_MP_PORT_END})"
    )


class VerlPortSafeMultiprocExecutor(MultiprocExecutor):
    """MultiprocExecutor using the port reserved by its Ray server actor."""

    def _init_executor(self) -> None:
        reserved_port = os.environ.get(_VERL_VLLM_MP_PORT_ENV)
        if reserved_port is None:
            return super()._init_executor()

        # MultiprocExecutor imports get_open_port directly into its module, so
        # patch that one call inside the spawned EngineCore process.  Restoring
        # it after workers are ready keeps every other vLLM port allocation on
        # its normal path.
        import vllm.v1.executor.multiproc_executor as multiproc_executor_module

        original_get_open_port = multiproc_executor_module.get_open_port
        multiproc_executor_module.get_open_port = lambda: int(reserved_port)
        try:
            return super()._init_executor()
        finally:
            multiproc_executor_module.get_open_port = original_get_open_port
...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        Args:
            ...
            name_suffix (str): rollout role suffix used to isolate local ports.
        """
        ...
        # Reserve the local TP/PP worker rendezvous port before replicas begin
        # their concurrent cold start.  The seed spreads preferred ports while
        # the live sockets guarantee uniqueness if ranges overlap.
        role_name = name_suffix.lstrip("_")
        role_slot = {"atk": 0, "def": 1, "old_atk": 2, "old_def": 3}.get(role_name, 4)
        # Reserve disjoint preferred ranges for the four co-training roles.
        # Ray may place different role pools on the same physical node, so a
        # replica-rank-only seed is insufficient even though GPU sets differ.
        mp_port_seed = role_slot * 512 + replica_rank * max(1, nnodes) + node_rank
        self._vllm_mp_port, self._vllm_mp_sock = _reserve_vllm_mp_port(mp_port_seed)
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        # Megatron requires CUDA_DEVICE_MAX_CONNECTIONS=1 but it causes vLLM V1
        # EngineCore subprocess to crash. Unset it for vLLM processes.
        ...
        if server_args.distributed_executor_backend == "mp":
            # The custom executor class is serialized with VllmConfig into the
            # spawned EngineCore process, where the actual port selection is
            # performed.  Environment variables are inherited by that process.
            os.environ[_VERL_VLLM_MP_PORT_ENV] = str(self._vllm_mp_port)
            server_args.distributed_executor_backend = VerlPortSafeMultiprocExecutor

        # All vLLM server actors have completed their port reservations by the
        # time launch_server is called.  Release only this actor's socket just
        # before EngineCore starts so its TCPStore can bind the reserved port.
        if self._vllm_mp_sock is not None:
            self._vllm_mp_sock.close()
            self._vllm_mp_sock = None
    ...
    def _apply_quantization(self) -> tuple[Optional[str], dict]:
        ...
        hf_overrides = dict(getattr(self.config, "hf_overrides", None) or {})
