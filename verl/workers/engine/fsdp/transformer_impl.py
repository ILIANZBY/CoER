...
class FSDPEngine(BaseEngine):
    ...
    def forward_backward_batch(self, data: TensorDict, loss_function: Callable, forward_only=False) -> list[TensorDict]:
        ...
        # FSDP2 CPUOffloadPolicy only manages parameters during forward/backward.
        # Buffers (e.g. rotary embedding inv_freq) may linger on CPU after explicit
        # offload calls (e.g. save_checkpoint → model.cpu()). Move them to GPU here.
        ...
        for micro_batch in micro_batches:
            # Invalidate gathered position_ids and cu_seqlens caches before each micro-batch
            # to avoid stale data across different micro-batches.
            from verl.models.transformers import monkey_patch as _mp
            _mp._gathered_position_ids_cache_val = None
            _mp._cached_cu_seqlens = None
    ...
    def save_checkpoint(
        ...
        **kwargs,
        ...
        # When using offload_policy (CPUOffloadPolicy), _is_offload_param is False but
        # parameters may still be on CPU after forward/backward.  Detect and move to GPU.
        needs_gpu_load = self._is_offload_param or origin_module_device == "cpu"
        if needs_gpu_load:
            load_fsdp_model_to_gpu(self.module)
        ...
        torch.distributed.barrier()
        if self._is_offload_param or needs_gpu_load:
            offload_fsdp_model_to_cpu(self.module)
    ...
    def get_per_tensor_param(self, layered_summon=False, base_sync_done=False, **kwargs):
        ...
        # For FSDP2 + offload_policy, skip model.to(device) as it corrupts internal DTensor state.
        # Use get_model_state_dict() which properly handles FSDP2 CPU offload.
        _use_fsdp2_offload = (
            self.engine_config.strategy == "fsdp2" and self.engine_config.offload_policy
        )
        if not _use_fsdp2_offload:
            load_fsdp_model_to_gpu(self.module)
        ...
        if hasattr(peft_model, "peft_config"):  # LoRA
            ...
            if _use_fsdp2_offload:
                from torch.distributed.checkpoint.state_dict import StateDictOptions, get_model_state_dict
                params = get_model_state_dict(
                    self.module,
...
class FSDPEngineWithLMHead(FSDPEngine):
    def prepare_model_inputs(self, micro_batch: TensorDict):
        ...
        if use_remove_padding:
            ...
            if self.use_ulysses_sp:
                ...
                # Only use the VLM path (pad-only, defer slicing to model forward) when
                # actual multi-modal inputs are present. For text-only inference on VLM
                # architectures (e.g., Qwen3.5 used without images), we must slice here
                # because the model forward won't do it.
                has_actual_mm_inputs = bool(multi_modal_inputs)
                if is_vlm_model and has_actual_mm_inputs:
                    ...
                    # For VLM architectures used in text-only mode (e.g., Qwen3.5 without images),
                    # position_ids may be 3D: (num_channels, 1, total_nnz) where num_channels=4.
                    # ulysses_pad_and_slice_inputs slices on dim=1, which would slice the
                    # size-1 dim to 0. We must skip position_ids in that call and slice it
                    # manually on dim=-1 (the seq dimension).
                    has_multidim_pos_ids = position_ids_rmpad.dim() == 3 and position_ids_rmpad.size(0) > 1
                    input_ids_rmpad, position_ids_rmpad_out, pad_size = ulysses_pad_and_slice_inputs(
                        input_ids_rmpad,
                        position_ids_rmpad=position_ids_rmpad,
                        sp_size=self.ulysses_sequence_parallel_size,
                        skip_position_ids_rmpad=has_multidim_pos_ids or self.__class__.__name__ == "VeOmniEngineWithLMHead",
                    )
                    if has_multidim_pos_ids:
                        # Pad position_ids on seq dim (last dim) then slice on last dim
                        position_ids_rmpad = ulysses_pad(
                            # ulysses_pad expects (bsz, seq) for first arg; reuse for padding logic
                            input_ids_rmpad=position_ids_rmpad.view(position_ids_rmpad.size(0), -1),
                            position_ids_rmpad=None,
                            sp_size=self.ulysses_sequence_parallel_size,
                        )[0]
                        position_ids_rmpad = slice_input_tensor(position_ids_rmpad, dim=-1, padding=False)
                        # Restore shape: (num_channels, 1, seq_len/sp)
                        position_ids_rmpad = position_ids_rmpad.unsqueeze(1)
                    else:
                        position_ids_rmpad = position_ids_rmpad_out
    ...
    def prepare_model_outputs(self, output, output_args, micro_batch: TensorDict, logits_processor_func):
        ...
        if use_remove_padding:
            ...
            if use_fused_kernels:
                ...
                # Avoid view + inplace on custom autograd outputs. Also skip the
                # full-vocab copy when temperature is 1.0, which is the common path
                # for PPO actor updates and saves several GB at long sequence length.
                if not torch.all(temperature_rmpad == 1):
    ...
    def forward_step(self, micro_batch: TensorDict, loss_function, forward_only):
        ...
        # Pre-compute cu_seqlens OUTSIDE any checkpointed region to guarantee determinism
        # during gradient checkpoint recomputation.  The checkpointed decoder layers will
        # read from the global cache without re-running all_gather or prepare_fa_kwargs.
        # Also do this for forward_only to avoid repeated all_gathers per layer and to
        # ensure flash_attn receives correct cu_seqlens for long gathered sequences.
        if self.use_ulysses_sp:
            from verl.models.transformers import monkey_patch as _mp
            from verl.utils.ulysses import (
                get_ulysses_sequence_parallel_group,
            ...
            pos_ids = model_inputs.get("position_ids")
            if pos_ids is not None:
                # Extract text_position_ids (first channel for Qwen3.5 4D position_ids)
                if pos_ids.ndim == 3 and pos_ids.size(0) >= 4:
                    text_pos_ids = pos_ids[0:1].reshape(1, -1)  # (1, seq_len/sp)
                else:
                    text_pos_ids = pos_ids  # (1, seq_len/sp) or (batch, seq_len/sp)
                sp_size = get_ulysses_sequence_parallel_world_size()
                pos_list = [torch.empty_like(text_pos_ids) for _ in range(sp_size)]
                torch.distributed.all_gather(
                    pos_list, text_pos_ids, group=get_ulysses_sequence_parallel_group()
                )
                gathered_pos = torch.cat(pos_list, dim=-1)
                from transformers.modeling_flash_attention_utils import prepare_fa_kwargs_from_position_ids
                (cu_q, cu_k), (max_q, max_k) = prepare_fa_kwargs_from_position_ids(gathered_pos)
                _mp._gathered_position_ids_cache_val = gathered_pos
                _mp._cached_cu_seqlens = (cu_q, cu_k, max_q, max_k)
