...
def postprocess_batch_func(output_lst, indices, data: TensorDict):
    ...
    for key, val in model_output.items():
        if pad_mode == DatasetPadMode.NO_PADDING:
            ...
            original_lengths = input_ids.offsets().diff().tolist() if input_ids.is_nested else None
            for micro_batch_idx, nt in enumerate(model_output[key]):
                try:
                    tensors.extend(nt.unbind())
                except RuntimeError:
                    if original_lengths is None or indices is None:
                        raise
                    micro_batch_indices = indices[micro_batch_idx]
                    micro_batch_lengths = [int(original_lengths[i]) for i in micro_batch_indices]
                    if nt.values().shape[0] != sum(micro_batch_lengths):
                        raise
                    tensors.extend(torch.split(nt.values(), micro_batch_lengths, dim=0))
