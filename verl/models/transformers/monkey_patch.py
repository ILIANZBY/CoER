...
# Cache for gathered position_ids AND pre-computed cu_seqlens to ensure determinism during
# gradient checkpoint recomputation.  Cleared at the start of each micro-batch in
# forward_backward_batch.
# Within one micro-batch (forward + backward recompute), position_ids content is always
# identical, so we compute cu_seqlens once and pass them directly to _flash_attention_forward,
# completely bypassing the data-dependent prepare_fa_kwargs_from_position_ids inside the
# checkpointed region.
_gathered_position_ids_cache_val = None
_cached_cu_seqlens = None  # tuple: (cu_seq_lens_q, cu_seq_lens_k, max_length_q, max_length_k)
...
def _ulysses_flash_attention_forward(
    ...
    **kwargs,
    ...
    if ulysses_sp_size > 1 and position_ids is not None:
        ...
        # Use cached gathered position_ids AND pre-computed cu_seqlens to ensure determinism
        # during gradient checkpoint recomputation.  The cache is populated in forward_step
        # BEFORE any checkpointed layer runs, so both forward and recompute see the same values.
        #
        # For forward-only inference (ref model, no_grad), the cache is empty.
        # In that case, do a fresh all_gather (no caching needed since no checkpoint).
        global _gathered_position_ids_cache_val, _cached_cu_seqlens

        if _gathered_position_ids_cache_val is not None:
            position_ids = _gathered_position_ids_cache_val
        else:
            position_ids_list = [torch.empty_like(position_ids) for _ in range(ulysses_sp_size)]
            torch.distributed.all_gather(position_ids_list, position_ids, group=get_ulysses_sequence_parallel_group())
            position_ids = torch.concat(position_ids_list, dim=-1)

        # NOTE: Do NOT compute or cache cu_seqlens here (inside the checkpointed region).
        # cu_seqlens are pre-computed in forward_step (outside checkpoint) when needed.

    # (bsz, seq_len, n_head/n, head_dim)
    query_length = query_states.size(1)

    # When ulysses SP is active and we have cached cu_seqlens, pass them explicitly so that
    # _flash_attention_forward uses the deterministic is_fa_with_varlen_kwargs path.
    extra_fa_kwargs = {}
    if _cached_cu_seqlens is not None:
        cu_q, cu_k, max_q, max_k = _cached_cu_seqlens
        extra_fa_kwargs = {
            "cu_seq_lens_q": cu_q,
            "cu_seq_lens_k": cu_k,
            "max_length_q": max_q,
        ...
        # Remove from kwargs to avoid "got multiple values" when unpacking both dicts.
        for k in list(extra_fa_kwargs.keys()):
            kwargs.pop(k, None)

    attn_output = _flash_attention_forward(
        query_states, key_states, value_states, attention_mask, query_length, *args,
        position_ids=position_ids, **extra_fa_kwargs, **kwargs
    )
