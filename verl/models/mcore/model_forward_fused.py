...
def fused_forward_model_engine(vision_model: bool = False):
    def fused_forward_model_engine_inner(
        ...
        input_ids_rmpad, packed_seq_params, packed_position_ids = preprocess_thd_engine(
            input_ids, pre_process=pre_process, use_fp8_padding=use_fp8_padding
        )
        input_ids_rmpad = input_ids_rmpad.contiguous()

        # ``preprocess_thd_engine`` already builds CP-aware, reset-per-sample
        # text positions.  Most GPT models derive their rotary positions from
        # sequence length, so fused inference historically discarded them.
        # Qwen3.5's MBridge text model uses multimodal RoPE even for text-only
        # inputs, where the rotary module requires explicit [3, batch, seq]
        # positions.  Preserve the calculated positions and replicate the
        # text axis for T/H/W; this is the text-only M-RoPE convention.
        unwrapped_model = unwrap_model(model)
        if getattr(unwrapped_model, "mrope_section", None) is not None:
            position_ids = packed_position_ids.unsqueeze(0).expand(3, -1, -1).contiguous()
            unwrapped_model.rotary_pos_emb.is_thd_format = True
        else:
            position_ids = None
