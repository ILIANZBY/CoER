...
class CheckpointEngineWorker(Worker):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Historical replicas are restarted during population restore.  A
        # failed rendezvous must not block the entire co-training job forever;
        # the population manager can disable/retry that role when this bounded
        # initialization raises.
        initialize_global_process_group_ray(timeout_second=180, backend="cpu:gloo")
...
class CheckpointEngineManager:
    ...
    async def update_weights(self, global_steps: int = None):
        ...
        # Skip NCCL sync when no replicas are available (e.g., cotrain uses file-based refresh)
