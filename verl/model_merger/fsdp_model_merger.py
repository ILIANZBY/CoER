...
class FSDPModelMerger(BaseModelMerger):
    ...
    def _calculate_shard_configuration(
        ...
        elif mesh_dim_names == ("fsdp",) and mesh.shape == (1,) and int(mesh[0]) > 1:
            # Non-DTensor FSDP checkpoints infer the mesh as np.array([world_size]).
            # In that case mesh.shape[-1] is 1, but we still need to load all ranks.
            total_shards = int(mesh[0])
            mesh_shape = (total_shards,)
        else:
    ...
    def _load_and_merge_state_dicts(
        ...
        for rank in tqdm(range(total_shards), desc=f"Loading {total_shards} FSDP shards", total=total_shards):
            append_one_shard(rank)
