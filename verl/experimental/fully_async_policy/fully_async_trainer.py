...
def _is_critic_only_warmup(global_steps: int, critic_warmup: int) -> bool:
    """Return whether actor updates are still disabled for critic warmup."""
    return global_steps < critic_warmup


def _should_transfer_actor_weights(
    *, global_steps: int, critic_warmup: int, initial_weight_sync_completed: bool
) -> bool:
    """Transfer once at startup, then only after actor updates are enabled."""
    return not initial_weight_sync_completed or not _is_critic_only_warmup(global_steps, critic_warmup)


def _can_accept_over_lag_sample_during_critic_warmup(
    *,
    global_steps: int,
    critic_warmup: int,
    sample_start_version: int,
) -> bool:
    """Allow bookkeeping-stale samples while the underlying actor is frozen.

    The fully async trainer advances checkpoint/policy version counters during
    critic-only warmup even though it does not update the actor.  Samples at or
    after initialization therefore come from identical actor weights and are
    safe to consume despite exceeding the normal policy-lag limit.
    """
    if not _is_critic_only_warmup(global_steps, critic_warmup):
        return False
    return sample_start_version >= 0
...
class FullyAsyncTrainer(SeparateRayPPOTrainer):
    ...
    def __init__(
        ...
        # Import custom modules (e.g. custom advantage estimators) specified via env var.
        # This is needed because Ray Actors run in separate processes where module-level
        # registrations from the driver process are not available.
        import importlib

        custom_modules = os.environ.get("VERL_CUSTOM_MODULES", "")
        if custom_modules:
            for mod_name in custom_modules.split(","):
                mod_name = mod_name.strip()
                if mod_name:
                    importlib.import_module(mod_name)
                    print(f"[FullyAsyncTrainer] Imported custom module: {mod_name}")
        ...
        self.stale_trajectory_dropped = 0
        self.policy_version_trajectory_dropped = 0
        self.warmup_over_lag_trajectory_accepted = 0
        self.current_param_version = 0
        self.total_train_steps = None
        self.progress_bar = None
        self.trigger_parameter_sync_step = config.async_training.trigger_parameter_sync_step
        self.last_ckpt_version = 0
        self.initial_weight_sync_completed = False
    ...
    def set_cotrain_role(self, role: str):
        """Set the cotrain role (attacker/defender) for population refresh signaling."""
        self.cotrain_role = role

    def get_current_param_version(self) -> int:
        return self.current_param_version

    def setup_checkpoint_manager_with_replicas(self, replicas):
        """Override checkpoint manager with specific replicas for NCCL sync.

        Used by co-training orchestrator to wire each trainer to its own
        set of vLLM replicas (attacker trainer → attacker replicas, etc.).
        """
    ...
    def get_rollout_addresses(self) -> list[str]:
        """Return HTTP addresses of current vLLM replicas managed by LLMServerManager.

        Only available when use_trainer_do_validate=True. Raises AttributeError otherwise
        so callers can catch it and fall back to configured defaults.
        """
        if self.llm_server_manager is None:
            raise AttributeError("LLMServerManager not initialized (use_trainer_do_validate=False)")
        return self.llm_server_manager.get_addresses()
    ...
    async def _get_samples_from_queue(self) -> tuple[None, None] | tuple[int, Any]:
        ...
        while len(queue_samples) < self.required_samples:
            ...
            decoded_sample = ray.cloudpickle.loads(sample)
            if self.cotrain_role and self._is_stale_cotrain_sample(decoded_sample):
                self.policy_version_trajectory_dropped += len(decoded_sample.full_batch)
                continue
            queue_samples.append(decoded_sample)
    ...
    def _is_stale_cotrain_sample(self, rollout_sample) -> bool:
        """Fail closed on missing, future, or over-lagged role policy metadata."""
        role = self.cotrain_role
        if role not in ("attacker", "defender"):
            return False
        trajectory_count = len(rollout_sample.full_batch)
        non_tensor_batch = rollout_sample.full_batch.non_tensor_batch
        start_key = f"{role}_policy_version"
        end_key = f"{role}_policy_version_end"
        if start_key not in non_tensor_batch or end_key not in non_tensor_batch:
            print(f"[FullyAsyncTrainer] Dropping {role} sample without policy-version metadata")
            return True
        try:
            start_version = min(int(value) for value in non_tensor_batch[start_key])
            end_version = max(int(value) for value in non_tensor_batch[end_key])
        except (TypeError, ValueError):
            print(f"[FullyAsyncTrainer] Dropping {role} sample with malformed policy-version metadata")
            return True
        if start_version < 0 or end_version < start_version or end_version > self.current_param_version:
            print(
                f"[FullyAsyncTrainer] Dropping {role} sample with invalid version span "
                f"[{start_version}, {end_version}] at trainer version {self.current_param_version}"
            )
            return True
        max_policy_lag = int(self.config.cotrain.get("max_policy_lag", 1))
        stale = self.current_param_version - start_version > max_policy_lag
        critic_warmup = int(self.config.trainer.critic_warmup)
        if stale and _can_accept_over_lag_sample_during_critic_warmup(
            global_steps=self.global_steps,
            critic_warmup=critic_warmup,
            sample_start_version=start_version,
        ):
            self.warmup_over_lag_trajectory_accepted += trajectory_count
            return False
        if stale:
            self.stale_trajectory_dropped += trajectory_count
            print(
                f"[FullyAsyncTrainer] Dropping stale {role} sample span "
                f"[{start_version}, {end_version}] at trainer version {self.current_param_version} "
    ...
    def _init_models(self):
        if self.use_critic:
            ...
            # TrainingWorker 在 __init__ 中已构建模型，
            # 需 reset() 初始化 engine + set_loss_fn() 设置 value loss
    ...
    async def fit_step(self, batch_dict: dict = None):
        ...
        await self._fit_validate()
        checkpoint_saved = self._fit_save_checkpoint()
        if checkpoint_saved:
            await self._fit_refresh_population()
    ...
    async def _fit_update_weights(self):
        ...
        critic_warmup = int(self.config.trainer.critic_warmup)
        should_transfer_weights = _should_transfer_actor_weights(
            global_steps=self.global_steps,
            critic_warmup=critic_warmup,
            initial_weight_sync_completed=self.initial_weight_sync_completed,
        )
        if not should_transfer_weights:
            # The actor has not changed.  Avoid aborting/resuming every in-flight
            # rollout merely to retransmit identical weights.  We still advance
            # the rollouter's logical version below so checkpoint cadence and
            # the warmup resume anchor retain their existing semantics.
            self.timing_raw["timing_s/param_sync"] = 0.0
            print(
                "[FullyAsyncTrainer] Skipping parameter transfer during critic-only warmup, "
                f"global_steps={self.global_steps}, actor_starts_at={critic_warmup}"
            )
        else:
            with marked_timer("timing_s/param_sync", self.timing_raw):
                await self.checkpoint_manager.update_weights(global_steps=self.current_param_version)
            self.initial_weight_sync_completed = True
        print(
        ...
        if self.cotrain_role:
            # Advance only this role's rollout version.  The other queue retains
            # its own freshness boundary and cannot be reset by this trainer.
            reset_future = self.rollouter.reset_staleness.remote(
                trainer_role=self.cotrain_role,
                param_version=self.current_param_version,
            )
        else:
            reset_future = self.rollouter.reset_staleness.remote()
        timing_raw = await asyncio.wrap_future(reset_future.future())
        self.logger.log(
            data=timing_raw,
    ...
    async def _fit_refresh_population(self):
        """Refresh historical opponents only after this version is on disk."""
        if not self.cotrain_role or self.current_param_version <= 0:
            return
        await asyncio.wrap_future(
            self.rollouter.maybe_refresh_old_model.remote(
                trainer_role=self.cotrain_role,
                param_version=self.current_param_version,
            ).future()
        )
            ...
            metrics.update(
                {
                    ...
                    "fully_async/count/stale_trajectory_accepted": self.stale_trajectory_processed,
                    "fully_async/count/stale_trajectory_dropped": self.stale_trajectory_dropped,
                    "fully_async/count/policy_version_trajectory_dropped": (
                        self.policy_version_trajectory_dropped
                    ),
                    "fully_async/count/warmup_over_lag_trajectory_accepted": (
                        self.warmup_over_lag_trajectory_accepted
                    ),
