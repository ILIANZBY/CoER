...
def addition_process(output: DataProto):
    """collect metirics"""
    metrics = output.meta_info.pop("metrics", None)
    if metrics is None:
        output.non_tensor_batch["processing_times"] = [0.0] * len(output)
        output.non_tensor_batch["tool_calls_times"] = [0.0] * len(output)
        return output
    if isinstance(metrics, dict):
        processing_times_list = metrics.get("generate_sequences", [0.0] * len(output))
        tool_calls_times_list = metrics.get("tool_calls", [0.0] * len(output))
    else:
        processing_times_list = [item["generate_sequences"] if isinstance(item, dict) else 0.0 for item in metrics]
        tool_calls_times_list = [item["tool_calls"] if isinstance(item, dict) else 0.0 for item in metrics]
...
def assemble_batch_from_rollout_samples(
    ...
    # Unify non_tensor_batch keys across all samples before concat.
    # Different agent loop modes (curr_curr vs clean vs fixed_template) may produce
    # different extra_fields keys. DataProto.concat requires all dicts share the same keys.
    all_ntb_keys: set = set()
    for b in rollout_samples_batch:
        all_ntb_keys.update(b.non_tensor_batch.keys())
    for b in rollout_samples_batch:
        for key in all_ntb_keys:
            if key not in b.non_tensor_batch:
                b.non_tensor_batch[key] = np.array([None] * b.batch.batch_size[0], dtype=object)

    final_batch = DataProto.concat(rollout_samples_batch)

    # Truncate batch to be divisible by ppo_mini_batch_size to avoid make_iterator assert
    if config is not None:
        mini_bs = config.actor_rollout_ref.actor.ppo_mini_batch_size
        total = final_batch.batch.batch_size[0]
        remainder = total % mini_bs
        if remainder != 0:
            keep = total - remainder
            final_batch = final_batch[:keep]
            print(f"[BatchUtils] Truncated batch from {total} to {keep} (divisible by {mini_bs})")
