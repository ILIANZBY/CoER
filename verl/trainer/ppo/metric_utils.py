...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Log reward_extra_info metrics (domain-specific metrics from agent loops)
    reward_extra_keys = batch.meta_info.get("reward_extra_keys", []) if hasattr(batch, "meta_info") else []
    for key in reward_extra_keys:
        if key in batch.non_tensor_batch:
            values = batch.non_tensor_batch[key]
            try:
                numeric_values = np.array(values, dtype=np.float64)
                finite_values = numeric_values[np.isfinite(numeric_values)]
                if len(finite_values) > 0:
                    metrics[f"{key}/mean"] = float(finite_values.mean())
