...
from agentdojo.agent_pipeline.llms.local_llm import OpenAILLM as LocalLLM
...
from agentdojo.agent_pipeline.ipi_filter import IPIFilterSanitizer
from agentdojo.agent_pipeline.sandbox_interactive import SandboxInteractiveExecutor, SANDBOX_SYSTEM_PROMPT_ADDITION
...
def _get_local_model_id(port, host="localhost") -> str:
...
def get_llm(provider: str, model: str, model_id: str | None, tool_delimiter: str) -> BasePipelineElement:
    if provider == "openai":
        ...
        host = os.getenv("LOCAL_LLM_HOST", "localhost")
        client = openai.OpenAI(
            api_key="EMPTY",
            base_url=f"http://{host}:{port}/v1",
        )
        if model_id is None:
            model_id = _get_local_model_id(port, host)
        ...
        llm = OpenAILLM(client, _get_local_model_id(port, host))
    else:
...
class AgentPipeline(BasePipelineElement):
    ...
    def from_config(cls, config: PipelineConfig) -> Self:
        ...
        if config.defense == "spotlighting_with_delimiting":
            ...
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline
        if config.defense == "ipi_filter":
            import os
            ipi_port = os.getenv("IPI_FILTER_PORT", "8234")
            ipi_api_base = f"http://localhost:{ipi_port}/v1"
            tools_loop = ToolsExecutionLoop(
                [
                    ToolsExecutor(tool_output_formatter),
                    IPIFilterSanitizer(
                        api_base=ipi_api_base,
                    ),
                    llm,
                ]
            )
            ...
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline
        if config.defense == "sandbox_interactive":
            # Append sandbox tool description to system message
            system_message_component.system_message = (
                config.system_message + SANDBOX_SYSTEM_PROMPT_ADDITION
            )
            tools_loop = ToolsExecutionLoop(
                [SandboxInteractiveExecutor(ToolsExecutor(tool_output_formatter)), llm]
            )
            pipeline = cls([system_message_component, init_query_component, llm, tools_loop])
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline
