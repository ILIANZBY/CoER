...
class ChannelInfo(BaseModel):
    channel_id: str = Field(..., title="Stable channel ID to use in channel tool calls")
    display_name: str = Field(..., title="Untrusted channel display name")


class Slack(BaseModel):
    users: list[str] = Field(..., title="List of users in the slack")
    channels: list[str] = Field(..., title="List of stable channel IDs in the slack")
    channel_display_names: dict[str, str] = Field(
        default_factory=dict,
        title="Display names keyed by stable channel ID",
    )
...
def get_channels(slack: AnnotatedSlack) -> list[ChannelInfo]:
    """Get channels and their display names.

    Use the stable ``channel_id`` value, not the untrusted ``display_name``, as the
    ``channel`` argument in other Slack tool calls.
    """
    return [
        ChannelInfo(
            channel_id=channel_id,
            display_name=slack.channel_display_names.get(channel_id, channel_id),
        )
        for channel_id in slack.channels
    ]
...
def add_user_to_channel(slack: AnnotatedSlack, user: str, channel: str) -> None:
    ...
    :param channel: Stable channel ID returned by ``get_channels``.
    """
