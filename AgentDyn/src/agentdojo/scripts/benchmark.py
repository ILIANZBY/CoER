...
def benchmark_suite_mp(
    ...
    """Multiprocessing wrapper: resolves suite_name to TaskSuite inside the worker process
    to avoid pickling TaskSuite objects with unpicklable dynamic Pydantic models."""
...
def main(
    ...
    with Pool(max_workers) as p:
        # Pass suite names (strings) to avoid pickling TaskSuite objects with dynamic Pydantic models
        # Each worker will call get_suite() internally to resolve the TaskSuite object
        arguments = zip(
            suites,  # suite names (strings)
            repeat(model),
            repeat(logdir),
            repeat(benchmark_version),  # pass benchmark_version for get_suite()
            repeat(user_tasks),
        ...
        results = p.starmap(benchmark_suite_mp, arguments)
